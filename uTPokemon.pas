...
public
  ...
  //Kampf-Funktionen & -Prozeduren - von Tobias
  procedure attackPkmn(Attacke:TAttacke);
  function prooveFightEndurance:Boolean;
...
procedure TPokemon.attackPkmn(Attacke:TAttacke);
begin
  if getcurrentHP < Attacke.Staerke             //wenn HP weniger als St�rke d. Attacke
    then setcurrentHP(0)                        //dann: HP auf 0 (=tot)
    else setcurrentHP(getcurrentHP-Attacke.Staerke);    //sonst: HP - St�rke d. Attacke
end;

function TPokemon.prooveFightEndurance:Boolean;  //solange HP nicht 0, kann weiterk�mpfen
begin
  if getcurrentHP = 0 then begin
                             FCanFight:=false;
                             Result:=false;
                           end
                      else Result:=true;
